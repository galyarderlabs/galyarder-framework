...
Galyarder exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`GALYARDER_API_KEY`)
...
- Run audit trail: include `X-Galyarder-Run-Id` header on all mutating requests during heartbeats
