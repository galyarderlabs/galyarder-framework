...
Add first-class **documents** to Galyarder as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Galyarder workflow/docs.
...
Update the Galyarder agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/galyarder/SKILL.md`
