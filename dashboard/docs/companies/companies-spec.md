...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Galyarder.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.galyarder.yaml`
...
- Galyarder must not require extra top-level fields for skill validity
- Galyarder-specific extensions must live under `metadata.galyarder` or `metadata.sources`
...
For Galyarder, the preferred fidelity extension is:
...
- issue/task Galyarder-only metadata
...
- Galyarder tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Galyarder shape:
...
Additional rules for Galyarder exporters:
...
- omit empty and default-valued Galyarder fields when possible
...
- Galyarder exporters should emit `.galyarder.yaml` as a sidecar by default
...
## 19. Galyarder Mapping

Galyarder can map this spec to its runtime model like this:
...
- Galyarder extension:
  - `.galyarder.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Galyarder-specific fidelity

Inline Galyarder-only metadata that must live inside a shared markdown file should use:

- `metadata.galyarder`

That keeps the base format broader than Galyarder.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Galyarder.

## 20. Cutover

Galyarder should cut over to this markdown-first package model as the primary portability format.

`galyarder.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Galyarder, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
