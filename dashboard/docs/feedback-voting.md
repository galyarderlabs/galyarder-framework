...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Galyarder saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
pnpm galyarder feedback report
```
...
```bash
# Installed CLI
galyarder feedback report

# Point to a different server or company
pnpm galyarder feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>

# Include raw payload dumps in the report
pnpm galyarder feedback report --payloads
```
...
```bash
pnpm galyarder feedback export
```
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # Galyarder feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the Galyarder envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
pnpm galyarder feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Default endpoint: when no feedback export backend URL is configured, Galyarder falls back to `https://telemetry.galyarder.ing`
