...
Galyarder companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .galyarder.yaml     # Adapter config, env inputs, routines
```
...
- **.galyarder.yaml** holds Galyarder-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
galyarder company export <company-id> --out ./my-export
```
...
galyarder company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
galyarder company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
galyarder company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.galyarder.yaml`
...
```sh
# From a local folder
galyarder company import ./my-export

# From a GitHub URL
galyarder company import https://github.com/org/repo

# From a GitHub subfolder
galyarder company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
galyarder company import org/repo
galyarder company import org/repo/companies/acme
```
...
If `--target` is not specified, Galyarder infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
galyarder company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
galyarder company import org/company-templates/engineering-team \
  --target new \
...
```sh
galyarder company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
galyarder company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
galyarder company import ./package \
  --target new \
  --yes \
  --json
```
...
Galyarder supports several GitHub URL formats:
