...
Low-level reference for how Galyarder packages are prepared and published to npm.
...
Galyarder no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `galyarder`, imports code from workspace packages such as:
...
- For an organization-scoped package like `@paperclipai/ui`, the `galyarder` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Galyarder uses calendar versions:
