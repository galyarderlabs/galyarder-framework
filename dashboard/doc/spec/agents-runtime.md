...
For `claude_local` and `codex_local`, Galyarder assumes the CLI is already installed and authenticated on the host machine.
...
Galyarder stores resumable session state per `(agent, taskKey, adapterType)`.
