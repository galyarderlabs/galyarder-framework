...
This spec defines how Galyarder actually runs agents while staying runtime-agnostic.
...
1. Galyarder is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Galyarder manages lifecycle, and on exit Galyarder parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `galyarder.skill` (shared Galyarder skill text block)
...
2. Preferred transport is env vars (`GALYARDER_*`) injected at runtime.
