...
Maintainer runbook for shipping Galyarder across npm, GitHub, and the website-facing changelog surface.
...
Galyarder uses calendar versions that still fit semver syntax:
...
2. **npm** — `galyarder` and public workspace packages are published
...
```bash
npx galyarder@canary onboard
# or
npx galyarder@canary onboard --data-dir "$(mktemp -d /tmp/galyarder-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/galyarder/galyarder/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Galyarder. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
GALYARDERAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable GALYARDERAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f galyarder_version=canary
gh workflow run release-smoke.yml -f galyarder_version=latest
```

Minimum checks:

- `npx galyarder@canary onboard` installs
