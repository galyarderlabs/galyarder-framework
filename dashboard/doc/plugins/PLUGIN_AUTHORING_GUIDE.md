...
This guide describes the current, implemented way to create a Galyarder plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Galyarder app.
...
node packages/plugins/create-galyarder-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Galyarder repo:

```bash
pnpm --filter @paperclipai/create-paperclip-plugin build
node packages/plugins/create-galyarder-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/galyarder/packages/plugins/sdk
```
...
Outside this monorepo, the scaffold snapshots `@paperclipai/plugin-sdk` from the local Galyarder checkout into a `.galyarder-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Galyarder from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
