# Galyarder Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Galyarder's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Galyarder app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Galyarder HTTP APIs directly.
...
Galyarder plugin design is based on the following assumptions:

1. Galyarder is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Galyarder business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Galyarder core.
...
2. Let plugins add major capabilities without editing Galyarder core.
...
4. Depend on project-local plugin folders such as `.galyarder/plugins`.
...
The single Galyarder deployment an operator installs and controls.

### 5.2 Company

A first-class Galyarder business object inside the instance.
...
A trusted in-process extension loaded directly by Galyarder core.
...
An installable instance-wide extension package loaded through the Galyarder plugin runtime.
...
Galyarder has two extension classes.
...
Galyarder already has a concrete workspace model:
...
Plugins live under the Galyarder instance directory.

Suggested layout:

- `~/.galyarder/instances/default/plugins/package.json`
- `~/.galyarder/instances/default/plugins/node_modules/`
- `~/.galyarder/instances/default/plugins/.cache/`
- `~/.galyarder/instances/default/data/plugins/<plugin-id>/`
...
Galyarder should add CLI commands:

- `pnpm galyarder plugin list`
- `pnpm galyarder plugin install <package[@version]>`
- `pnpm galyarder plugin uninstall <plugin-id>`
- `pnpm galyarder plugin upgrade <plugin-id> [version]`
- `pnpm galyarder plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@galyarder/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface GalyarderPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumGalyarderVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Galyarder agents can use during runs.
...
- Galyarder server starts one worker process per installed plugin
...
Receives one typed Galyarder domain event.
...
export function definePlugin(definition: PluginDefinition): GalyarderPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: GalyarderPluginManifestV1;
  config: {
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Galyarder secret provider system rather than a plain text input
...
1. Core Galyarder data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Galyarder objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Galyarder product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Galyarder later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Galyarder secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm galyarder plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Galyarder server. This is a normative requirement, not optional.
...
For developing a plugin against a running Galyarder instance:

- The operator installs the plugin from a local path: `pnpm galyarder plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-galyarder-plugin`) that scaffolds:

- `package.json` with correct `galyarderPlugin` keys
...
- `@galyarder/plugin-workspace-files`
- `@galyarder/plugin-terminal`
- `@galyarder/plugin-git`
- `@galyarder/plugin-linear`
- `@galyarder/plugin-github-issues`
- `@galyarder/plugin-grafana`
- `@galyarder/plugin-runtime-processes`
- `@galyarder/plugin-stripe`
...
- `create-galyarder-plugin` starter template
...
Galyarder should not implement a generic in-process hook bag modeled directly after local coding tools.

Galyarder should implement:
...
That is the complete target design for the Galyarder plugin system.
