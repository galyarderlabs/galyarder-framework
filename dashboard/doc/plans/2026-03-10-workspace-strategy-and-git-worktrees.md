...
`PAP-447` asks how Galyarder should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Galyarder instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Galyarder:

- some operators use Galyarder to manage Galyarder and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Galyarder
- Claude and Codex expose different built-in affordances, so Galyarder should not overfit to one tool

## Core Product Decision

Galyarder should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Galyarder
...
- Galyarder should not assume that every adapter can see or use a host filesystem path directly
...
If Galyarder makes worktrees agent-first, it will blur:
...
- Galyarder core resolves and records execution workspace state
...
That gives Galyarder room to support:
...
- Galyarder handles workspace runtime services under the hood using project/workspace policy
...
Once Galyarder is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Galyarder should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Galyarder cannot infer
...
- when issue moves to done, should Galyarder auto-commit?
- should it auto-push?
...
Galyarder already has the right foundation for a project-first model.
...
For server-spawned local adapters, Galyarder already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Galyarder should mirror directly

Implication:

- for `codex_local`, Galyarder should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Galyarder should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Galyarder should separate:
...
Galyarder should be able to express intentions such as:
...
These are not specific to Galyarder. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".galyarder/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Galyarder-specific `devServer` flag.
...
Galyarder should distinguish between:
...
Galyarder should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Galyarder should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Galyarder
...
For runtime services specifically, Galyarder should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Galyarder-only dev-server flags
...
3. Inject resolved execution workspace details into `context.galyarderWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Galyarder instance can be reused through env/config injection when appropriate
...
- Galyarder can decide whether to reuse or start a fresh service deterministically
...
3. Test against Galyarder-on-Galyarder workflows first.
...
7. Workspace runtime services are modeled generically, not as Galyarder-specific dev-server toggles.
