...
This document defines the rollout plan for adapter-wide skill support in Galyarder.
...
- Galyarder stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Galyarder currently has these adapters:
...
1. Desired skills live in Galyarder for every adapter.
...
These adapters have a stable local skills directory that Galyarder can read and manage.
...
These adapters do not have a meaningful Galyarder-owned persistent install state.
...
- desired skills stored in Galyarder
...
- runtime already injects Galyarder skills into `~/.cursor/skills`
...
- runtime already injects Galyarder skills into `~/.gemini/skills`
...
- runtime already injects Galyarder skills into `~/.pi/agent/skills`
...
- Galyarder can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Galyarder skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Galyarder may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Galyarder skills that are clearly marked as Galyarder-managed
...
- Galyarder stores desired skills only
...
- avoid destructive wording unless Galyarder can prove a skill is Galyarder-managed
...
That gets Galyarder from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
