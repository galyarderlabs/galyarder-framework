...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/galyarder-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `GALYARDER_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `GALYARDER_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Galyarder may create host-local worktrees, processes, and paths
- `cloud`
  - Galyarder should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `galyarder | github | vercel | s3 | custom`
...
- use `GALYARDER_EXECUTION_TOPOLOGY` as a defaulting guardrail
