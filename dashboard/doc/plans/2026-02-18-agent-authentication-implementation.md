...
- New config/env settings:
  - `GALYARDER_AGENT_JWT_SECRET`
  - `GALYARDER_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `GALYARDER_AGENT_JWT_ISSUER` (default: `galyarder`)
  - `GALYARDER_AGENT_JWT_AUDIENCE` (default: `galyarder-api`)
...
1. In:
   ...
   inject `GALYARDER_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `GALYARDER_API_KEY`, do not overwrite it.
- Continue injecting:
  - `GALYARDER_AGENT_ID`
...
- Local adapters authenticate without manual `GALYARDER_API_KEY` config.
