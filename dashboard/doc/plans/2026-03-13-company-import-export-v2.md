...
This document defines the next-stage plan for Galyarder company import/export.

The core shift is:

- move from a Galyarder-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Galyarder.
...
Galyarder already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Galyarder
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `galyarder.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.galyarder.yaml` sidecar for high-fidelity Galyarder-specific details
...
11. adapter-aware skill sync surfaces so Galyarder can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Galyarder at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Galyarder sidecar.
...
Galyarder must not redefine `SKILL.md`.
...
- Galyarder-specific fidelity lives in `.galyarder.yaml`
- Galyarder may resolve and install `SKILL.md` packages, but it must not require a Galyarder-only skill format
...
### 5.4 Base Package Vs Galyarder Extension
...
- base package:
  ...
  - no Galyarder-only runtime fields by default
- Galyarder extension:
  - `.galyarder.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Galyarder tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`galyarder.manifest.json` is not part of the future package direction.
...
Galyarder import/export should support these entity kinds:
...
- Galyarder can treat “this set of agents came from team package X” as the imported-team model
...
Galyarder should support source references in package metadata with:
...
Galyarder should define a new adapter surface area around skills:
...
Planned Galyarder behavior:

- if an adapter supports read, Galyarder should show current skills in the UI
- if an adapter supports write, Galyarder should let the user enable/disable imported skills
- if an adapter supports sync, Galyarder should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.galyarder.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.galyarder.yaml` alongside the base package
...
## 10. Storage Model Inside Galyarder
...
Galyarder should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `galyarder company export <company-id> --out <path>`
- `galyarder company import <path-or-url> --dry-run`
- `galyarder company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Galyarder storage, or only referenced at import time?
...
3. Should Galyarder support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Galyarder aligned with:
