...
Galyarder needs to support two very different but equally valid ways of working:
...
Today, Galyarder already has the beginnings of this model:
...
- making Galyarder feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Galyarder users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Galyarder deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Galyarder into a full code review product
...
This object must be recorded explicitly so that Galyarder can:
...
Galyarder should treat PRs as a type of work product linked back to:
...
Galyarder already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Galyarder must be a first-class deployment mode

Galyarder cannot assume that it is running on the same machine as the code.

In cloud deployments, Galyarder may:
...
- `Runtime service`: a process or service Galyarder owns or tracks for a workspace
...
It also lets cloud-hosted Galyarder deployments point at codebases and remotes without pretending the Galyarder host has direct filesystem access.
...
Without an explicit `execution workspace` record, Galyarder has nowhere stable to attach:
...
Galyarder needs a single place to show:
...
- Galyarder-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Galyarder owning the execution host
...
## 1. Galyarder host and execution host must be decoupled
...
without the Galyarder host itself running local git or local preview processes.
...
Galyarder should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Galyarder.
...
Galyarder should keep workspace automation centered on a small set of hooks:
...
Galyarder should provide the equivalent visibility in the project `Code` surface:
...
Galyarder should treat execution workspaces as the natural home for:
...
Galyarder should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Galyarder should continue treating cleanup policy as part of the core workspace model:
...
Galyarder should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Galyarder host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Galyarder should borrow Worktrunk's good execution patterns while keeping the broader Galyarder model:
...
This is how Galyarder supports:
...
That keeps Galyarder focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Galyarder deployments remain first-class

That is a better fit for Galyarder than either extreme:
