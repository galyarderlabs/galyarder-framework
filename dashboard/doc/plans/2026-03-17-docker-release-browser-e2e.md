...
Today release smoke testing for published Galyarder packages is manual and shell-driven:
...
- Docker
- `npx galyarder@canary`
- `npx galyarder@latest`
...
- runs `galyarder@${GALYARDERAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@galyarder.local`
- password: `galyarder-smoke-password`
...
- the suite passes locally against both:
  - `GALYARDERAI_VERSION=canary`
  - `GALYARDERAI_VERSION=latest`
...
1. A published `galyarder@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `galyarder@latest` Docker install can be smoke-tested by Playwright in CI.
