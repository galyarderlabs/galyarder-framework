 ...
 *    (`~/.galyarder/plugins/`) and `node_modules` for packages matching
 *    the `galyarder-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Galyarder Dashboard plugins.
 * Packages matching this pattern are considered Galyarder Dashboard plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "galyarder-plugin-";
...
export type PluginSource =
  | "local-filesystem"  // ~/.galyarder/plugins/ local directory
  | "npm"               // npm packages matching galyarder-plugin-* convention
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the galyarder-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "galyarder-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginLoader {
  ...
  /**
   * Discover Galyarder Dashboard plugins installed as npm packages in the current
   * Node.js environment matching the "galyarder-plugin-*" naming convention.
   *
   ...
   * the "galyarderPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Galyarder Dashboard plugin.
   * Throws if the package is a Galyarder Dashboard plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<GalyarderPluginManifestV1 | null>;
...
/**
 * Check whether a package name matches the Galyarder Dashboard plugin naming convention.
 * Accepts both the "galyarder-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
 ...
 *
 * The spec defines a "galyarderPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const galyarderPlugin = pkgJson["galyarderPlugin"];
  if (
    galyarderPlugin !== null &&
    typeof galyarderPlugin === "object" &&
    !Array.isArray(galyarderPlugin)
  ) {
    const manifestRelPath = (galyarderPlugin as Record<string, unknown>)[
      "manifest"
    ];
 ...
 *   packageName: "galyarder-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Galyarder Dashboard plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<GalyarderPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Galyarder Dashboard plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasGalyarderPlugin = "galyarderPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasGalyarderPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<GalyarderPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
