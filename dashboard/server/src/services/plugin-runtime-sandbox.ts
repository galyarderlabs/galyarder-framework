...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __galyarder_exports: module.exports,
      __galyarder_module: module,
      __galyarder_require: requireInSandbox,
      __galyarder_filename: realPath,
      __galyarder_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__galyarder_exports, __galyarder_module, __galyarder_require, __galyarder_filename, __galyarder_dirname)`;
