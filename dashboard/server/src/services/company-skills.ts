...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("galyarder/galyarder/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveGalyarderInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "galyarder_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Galyarder Dashboard skills are read-only.",
      sourceLabel: "Galyarder bundled",
      sourceBadge: "galyarder",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Galyarder workspace",
        sourceBadge: "galyarder",
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "galyarder_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Galyarder Dashboard skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "galyarder_bundled"
        && incomingKind === "github"
        && incomingOwner === "galyarder"
        && incomingRepo === "galyarder"
      ) {
        out.push(existing);
        continue;
      }
