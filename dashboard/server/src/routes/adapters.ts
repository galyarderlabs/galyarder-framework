...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-galyarder-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/hermes-galyarder-adapter@0.3.0" → packageName + version
  ...
  // without modifying Galyarder Dashboard's source code.
  //
