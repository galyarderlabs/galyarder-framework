...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "galyarder" &&
    normalized !== "galyarder-create-agent" &&
    normalized !== "galyarder-create-plugin" &&
...
/** Resolve the Galyarder Dashboard repo skills directory (built-in / managed skills). */
function resolveGalyarderSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const galyarderSkillsDir = resolveGalyarderSkillsDir();

  // Build set of Galyarder Dashboard-managed skill names
  const galyarderSkillNames = new Set<string>();
  if (galyarderSkillsDir) {
    try {
      for (const entry of fs.readdirSync(galyarderSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) galyarderSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isGalyarderManaged: galyarderSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.galyarderApiUrl)) {
    const legacyGalyarderApiUrl = nonEmptyTrimmedString(input.galyarderApiUrl);
    if (legacyGalyarderApiUrl) merged.galyarderApiUrl = legacyGalyarderApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    galyarderApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.galyarderApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawGalyarderApiUrl =
    typeof defaults.galyarderApiUrl === "string"
      ? defaults.galyarderApiUrl.trim()
      : "";
  if (rawGalyarderApiUrl) {
    try {
      const parsedGalyarderApiUrl = new URL(rawGalyarderApiUrl);
      if (
        parsedGalyarderApiUrl.protocol !== "http:" &&
        parsedGalyarderApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_galyarder_api_url_protocol",
          level: "warn",
          message: `galyarderApiUrl must use http:// or https:// (got ${parsedGalyarderApiUrl.protocol}).`
        });
      } else {
        normalized.galyarderApiUrl = parsedGalyarderApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_galyarder_api_url_configured",
          level: "info",
          message: `galyarderApiUrl set to ${parsedGalyarderApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_galyarder_api_url_invalid",
        level: "warn",
        message: `Invalid galyarderApiUrl: ${rawGalyarderApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Galyarder Dashboard host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Galyarder is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm galyarder allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/galyarder";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/galyarder-claimed-api-key.json and load GALYARDER_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: galyarderApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Galyarder Dashboard hostname is reachable and allowed via `pnpm galyarder allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Galyarder Dashboard API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/galyarder/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Galyarder Dashboard OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          galyarderApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Galyarder Dashboard generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "galyarderApiUrl": "https://galyarder-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Galyarder Dashboard before key claim is allowed.
    ...
    - ~/.openclaw/workspace/galyarder-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/galyarder-claimed-api-key.json

    And set the GALYARDER_API_KEY and GALYARDER_API_URL in your environment variables as specified here:
    ...
    Then set GALYARDER_API_KEY and GALYARDER_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Galyarder Dashboard skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your GALYARDER_API_URL to the top of your skill and note the path to your GALYARDER_API_URL
    ...
    ${
      ...
      "Ensure Galyarder Dashboard is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Galyarder Dashboard base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.galyarderApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm galyarder allowed-hostname <host>
      - then restart Galyarder Dashboard and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@galyarder.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "galyarder", path: "/api/skills/galyarder" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "galyarder-create-agent",
          path: "/api/skills/galyarder-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              galyarderApiUrl: req.body.galyarderApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.galyarderApiUrl &&
          !persistedDefaults.galyarderApiUrl
        ) {
          missingPersistedFields.push("galyarderApiUrl");
        }
