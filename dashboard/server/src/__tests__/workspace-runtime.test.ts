...
async function createTempRepo(defaultBranch = "main") {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "galyarder@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Galyarder Test"]);
...
afterEach(async () => {
  ...
  delete process.env.GALYARDER_CONFIG;
  delete process.env.GALYARDER_HOME;
  delete process.env.GALYARDER_INSTANCE_ID;
  delete process.env.GALYARDER_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Galyarder Dashboard and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/galyarder",
      GALYARDER_HOME: "/tmp/galyarder-home",
      GALYARDER_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.GALYARDER_HOME).toBeUndefined();
    expect(sanitized.GALYARDER_INSTANCE_ID).toBeUndefined();
...
describe("ensureServerWorkspaceLinksCurrent", () => {
  it("relinks stale server workspace dependencies inside the current repo root", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-links-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-links-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@galyarder");
  ...
  it("skips relinking when server workspace dependencies already point at the repo", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-links-current-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@galyarder");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".galyarder", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$GALYARDER_WORKSPACE_BRANCH\" > .galyarder-provision-branch",
        "printf '%s\\n' \"$GALYARDER_WORKSPACE_BASE_CWD\" > .galyarder-provision-base",
        "printf '%s\\n' \"$GALYARDER_WORKSPACE_CREATED\" > .galyarder-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".galyarder-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".galyarder-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".galyarder-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".galyarder-provision-created"), "utf8")).resolves.toBe("false\n");
  });
  ...
  it("uses the latest repo-managed provision script when reusing an existing worktree", async () => {
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".galyarder-provision-version"), "utf8")).resolves.toBe("v1\n");
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".galyarder-provision-version"), "utf8")).resolves.toBe("v2\n");
  });

  it("writes an isolated repo-local Galyarder Dashboard config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const galyarderHome = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(galyarderHome, "instances", instanceId);
    ...
    process.env.GALYARDER_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/galyarder"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".galyarder", "config.json");
      const envPath = path.join(workspace.cwd, ".galyarder", ".env");
      ...
      expect(envVars.GALYARDER_HOME).toBe(isolatedWorktreeHome);
      expect(envVars.GALYARDER_INSTANCE_ID).toBe(expectedInstanceId);
      expect(await fs.realpath(envVars.GALYARDER_CONFIG!)).toBe(await fs.realpath(configPath));
      expect(envVars.GALYARDER_IN_WORKTREE).toBe("true");
      expect(envVars.GALYARDER_WORKTREE_NAME).toBe("PAP-885-show-worktree-banner");

      process.chdir(workspace.cwd);
      expect(resolveGalyarderConfigPath()).toBe(configPath);
    } finally {
  ...
  it("provisions successfully when install is needed but there are no symlinked node_modules to move", async () => {
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".galyarder", "config.json"), "utf8")).resolves.toContain(
      "\"database\"",
    );
  }, 30_000);
  ...
  it("auto-detects the default branch when baseRef is not configured", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-worktree-bare-"));
  ...
  it("auto-detects the default branch via symbolic-ref when origin/HEAD is set", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-worktree-bare-symref-"));
    await runGit(bareRemote, ["init", "--bare"]);
    await runGit(repoRoot, ["remote", "add", "origin", bareRemote]);
    await runGit(repoRoot, ["push", "-u", "origin", "master"]);
    await runGit(repoRoot, ["fetch", "origin"]);
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-workspace-"));
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".galyarder", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.GALYARDER_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              GALYARDER_HOME: "{{workspace.cwd}}/.galyarder/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".galyarder", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".galyarder", "runtime-services"));
  });

  it("does not leak parent Galyarder Dashboard instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "galyarderConfig: process.env.GALYARDER_CONFIG ?? null,",
          "galyarderHome: process.env.GALYARDER_HOME ?? null,",
          "galyarderInstanceId: process.env.GALYARDER_INSTANCE_ID ?? null,",
    ...
    process.env.GALYARDER_CONFIG = "/tmp/base-galyarder-config.json";
    process.env.GALYARDER_HOME = "/tmp/base-galyarder-home";
    process.env.GALYARDER_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/galyarder";
    ...
    expect(captured.galyarderConfig).toBeNull();
    expect(captured.galyarderHome).toBeNull();
    expect(captured.galyarderInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-sibling-"));
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("galyarder-workspace-runtime-");
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-reconcile-"));
    const galyarderHome = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-home-"));
    process.env.GALYARDER_HOME = galyarderHome;
    process.env.GALYARDER_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("marks persisted local services stopped when the registry pid is stale", async () => {
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      serviceName: "galyarder-dev",
      status: "running",
      lifecycle: "shared",
      reuseKey: `project_workspace:${projectWorkspaceId}:galyarder-dev`,
      command: "pnpm dev",
      cwd: "/tmp/galyarder-primary",
      port: 49195,
    ...
    await writeLocalServiceRegistryRecord({
      version: 1,
      serviceKey: "workspace-runtime-galyarder-dev-stale",
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-runtime-stop-persisted-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const runId = randomUUID();
