...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "galyarder",
        name: "galyarder",
        description: "Galyarder coordination skill",
        markdown: "---\nname: galyarder\ndescription: Galyarder Dashboard coordination skill\n---\n\n# Galyarder Dashboard\n",
        sourceType: "github",
        sourceLocator: "https://github.com/galyarder/galyarder/tree/master/skills/galyarder",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/galyarder/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "galyarder",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/galyarder/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "galyarder",
  ...
  it("exports referenced skills as stubs by default with sanitized Galyarder Dashboard extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Galyarder"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${galyarderKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/galyarder/galyarder/galyarder/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/galyarder/galyarder/galyarder/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/galyarder/galyarder/galyarder/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".galyarder.yaml"]);
    expect(extension).toContain('schema: "galyarder/v1"');
    ...
    expect(extension).not.toContain("galyarderSkillSync");
  ...
  it("exports default sidebar order into the Galyarder Dashboard extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".galyarder.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/galyarder/galyarder/galyarder/SKILL.md"])).toContain("# Galyarder Dashboard");
    expect(asTextFile(exported.files["skills/galyarder/galyarder/galyarder/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/galyarder/galyarder/galyarder/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/galyarder/galyarder/galyarder/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/galyarder/galyarder/galyarder/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .galyarder.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".galyarder.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-galyarder",
        companyId: "company-1",
        key: "galyarder/galyarder/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/galyarder/galyarder/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "galyarder_bundled",
          owner: "galyarder",
          repo: "galyarder",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/galyarder/galyarder/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/galyarder/galyarder/release-changelog/SKILL.md"])).toContain("galyarder/galyarder/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/galyarder",
            repoUrl: "https://github.com/galyarder/galyarder.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .galyarder-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/galyarder/galyarder.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/galyarder");
    ...
    companySvc.create.mockResolvedValue({
      id: "company-imported",
      name: "Imported Galyarder Dashboard",
    });
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Galyarder Dashboard",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/galyarder/galyarder.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".galyarder.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/galyarder/galyarder.git"');
    expect(extension).toContain('projectWorkspaceKey: "galyarder"');
  ...
  it("reads env inputs back from .galyarder.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .galyarder.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Galyarder Dashboard routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "galyarder-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Galyarder Dashboard" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Galyarder Dashboard"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .galyarder.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Galyarder Dashboard");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".galyarder.yaml"] = `${exported.files[".galyarder.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
