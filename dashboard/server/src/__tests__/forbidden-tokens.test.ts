...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "galyarder", LOGNAME: "galyarder", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "galyarder" }),
      },
    );

    expect(tokens).toEqual(["galyarder", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "galyarder" }, {
        userInfo: () => ({ username: "galyarder" }),
      });

      expect(tokens).toEqual(["galyarder", "custom-token"]);
    } finally {
