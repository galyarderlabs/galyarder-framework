...
describe("agent instructions service", () => {
  const originalGalyarderHome = process.env.GALYARDER_HOME;
  const originalGalyarderInstanceId = process.env.GALYARDER_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalGalyarderHome === undefined) delete process.env.GALYARDER_HOME;
    else process.env.GALYARDER_HOME = originalGalyarderHome;
    if (originalGalyarderInstanceId === undefined) delete process.env.GALYARDER_INSTANCE_ID;
    else process.env.GALYARDER_INSTANCE_ID = originalGalyarderInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const galyarderHome = await makeTempDir("galyarder-agent-instructions-home-");
    const externalRoot = await makeTempDir("galyarder-agent-instructions-external-");
    cleanupDirs.add(galyarderHome);
    cleanupDirs.add(externalRoot);
    process.env.GALYARDER_HOME = galyarderHome;
    process.env.GALYARDER_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("galyarder-agent-instructions-new-external-");
    cleanupDirs.add(galyarderHome);
    cleanupDirs.add(externalRoot);
    process.env.GALYARDER_HOME = galyarderHome;
    process.env.GALYARDER_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("galyarder-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const galyarderHome = await makeTempDir("galyarder-agent-instructions-recover-");
    cleanupDirs.add(galyarderHome);
    process.env.GALYARDER_HOME = galyarderHome;
    process.env.GALYARDER_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const galyarderHome = await makeTempDir("galyarder-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("galyarder-agent-instructions-stale-root-");
    cleanupDirs.add(galyarderHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const galyarderHome = await makeTempDir("galyarder-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("galyarder-agent-instructions-heal-write-stale-");
    cleanupDirs.add(galyarderHome);
    cleanupDirs.add(staleRoot);
    process.env.GALYARDER_HOME = galyarderHome;
    process.env.GALYARDER_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const galyarderHome = await makeTempDir("galyarder-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("galyarder-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(galyarderHome);
    cleanupDirs.add(staleRoot);
    process.env.GALYARDER_HOME = galyarderHome;
    process.env.GALYARDER_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const galyarderHome = await makeTempDir("galyarder-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("galyarder-agent-instructions-partial-root-");
    cleanupDirs.add(galyarderHome);
    cleanupDirs.add(staleRoot);
    process.env.GALYARDER_HOME = galyarderHome;
    process.env.GALYARDER_INSTANCE_ID = "test-instance";
