...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Galyarder Dashboard skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("galyarder-cursor-skills-src-");
    const skillsHome = await makeTempDir("galyarder-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "galyarder");
    await createSkillDir(skillsDir, "galyarder-create-agent");
    ...
    const injectedA = path.join(skillsHome, "galyarder");
    const injectedB = path.join(skillsHome, "galyarder-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "galyarder")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "galyarder-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "galyarder"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "galyarder-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("galyarder-cursor-preserve-src-");
    const skillsHome = await makeTempDir("galyarder-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "galyarder");
    ...
    expect((await fs.lstat(path.join(skillsHome, "galyarder-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("galyarder-cursor-fail-src-");
    const skillsHome = await makeTempDir("galyarder-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
