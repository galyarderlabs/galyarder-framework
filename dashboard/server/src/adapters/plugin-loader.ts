...
function extractUiParserSource(
  ...
  const contractVersion = pkg.galyarder?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no galyarder.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
