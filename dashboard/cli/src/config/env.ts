...
const JWT_SECRET_ENV_KEY = "GALYARDER_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Galyarder environment variables",
    "# Generated by Galyarder CLI commands",
...
export function resolveGalyarderEnvFile(configPath?: string): string {
...
export function loadGalyarderEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeGalyarderEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readGalyarderEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeGalyarderEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeGalyarderEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readGalyarderEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeGalyarderEnvEntries(next, filePath);
  return next;
}
