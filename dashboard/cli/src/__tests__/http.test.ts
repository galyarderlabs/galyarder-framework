...
import { ApiConnectionError, ApiRequestError, GalyarderApiClient } from "../client/http.js";

describe("GalyarderApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new GalyarderApiClient({
    ...
    expect(headers["x-galyarder-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new GalyarderApiClient({ apiBase: "http://localhost:3100" });
