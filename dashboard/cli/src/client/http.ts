...
export class GalyarderApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-galyarder-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Galyarder API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Galyarder server is not running, the configured URL is wrong, or the request is being blocked before it reaches Galyarder.",
    "",
    "Try:",
    "- Start Galyarder with `pnpm dev` or `pnpm galyarder run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Galyarder is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`GALYARDER_API_URL\`.`,
  );
  return lines.join("\n");
}
