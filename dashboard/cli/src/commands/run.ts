...
export async function runCommand(opts: RunOptions): Promise<void> {
  const instanceId = resolveGalyarderInstanceId(opts.instance);
  process.env.GALYARDER_INSTANCE_ID = instanceId;

  const homeDir = resolveGalyarderHomeDir();
  ...
  loadGalyarderEnvFile(configPath);

  p.intro(pc.bgCyan(pc.black(" galyarder run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("galyarder onboard")} once, then retry ${pc.cyan("galyarder run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting Galyarder server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.GALYARDER_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@paperclipai/server/src/index.ts")) {
    process.env.GALYARDER_UI_DEV_MIDDLEWARE = "true";
  }
}
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  try {
    ...
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Galyarder server entrypoint.\n` +
          `Tried: ${devEntry}, @paperclipai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `Galyarder server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: GalyarderConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`Galyarder server entrypoint did not export startServer(): ${label}`);
  }
