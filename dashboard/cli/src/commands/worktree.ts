...
import type { GalyarderConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printGalyarderCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.GALYARDER_CONFIG;
...
const WORKTREE_NAME_PREFIX = "galyarder-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.GALYARDER_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.GALYARDER_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromGalyarderConfig(config: GalyarderConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromGalyarderConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.galyarder${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".galyarder", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".galyarder", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.galyarder"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: GalyarderConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://galyarder:galyarder@127.0.0.1:${port}/galyarder`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: GalyarderConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.GALYARDER_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.GALYARDER_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.GALYARDER_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.GALYARDER_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: GalyarderConfig;
  targetConfig: GalyarderConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveGalyarderEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readGalyarderEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://galyarder:galyarder@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "galyarder");
    const targetConnectionString = `postgres://galyarder:galyarder@127.0.0.1:${targetHandle.port}/galyarder`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readGalyarderEnvEntries(resolveGalyarderEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.GALYARDER_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.GALYARDER_AGENT_JWT_SECRET);
  mergeGalyarderEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { GALYARDER_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadGalyarderEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Galyarder inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printGalyarderCliBanner();
  p.intro(pc.bgCyan(pc.black(" galyarder worktree init ")));
...
function resolveCurrentWorktreeReseedState(opts: { home?: string } = {}) {
  ...
  if (!existsSync(currentConfigPath)) {
    throw new Error(
      "Current directory does not have a Galyarder worktree config. Run `galyarder worktree init` here first.",
    );
  }
  ...
  const currentEnvEntries = readGalyarderEnvEntries(resolveGalyarderEnvFile(currentConfigPath));
  ...
  return {
    ...
    instanceId:
      nonEmpty(currentEnvEntries.GALYARDER_INSTANCE_ID)
      ?? nonEmpty(path.basename(instanceRoot))
      ?? sanitizeWorktreeInstanceId(path.basename(process.cwd())),
    homeDir: path.resolve(expandHomePrefix(opts.home ?? currentEnvEntries.GALYARDER_HOME ?? derivedHomeDir)),
    serverPort: currentConfig.server.port,
    dbPort: currentConfig.database.embeddedPostgresPort,
    worktreeName: nonEmpty(currentEnvEntries.GALYARDER_WORKTREE_NAME) ?? undefined,
    worktreeColor: nonEmpty(currentEnvEntries.GALYARDER_WORKTREE_COLOR) ?? undefined,
  };
}
...
async function snapshotWorktreeReseedState(target: {
  ...
  const tempRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "galyarder-worktree-reseed-backup-"));
  return {
    tempRoot,
...
export async function worktreeReseedCommand(opts: WorktreeReseedOptions): Promise<void> {
  printGalyarderCliBanner();
  p.intro(pc.bgCyan(pc.black(" galyarder worktree reseed ")));
  ...
  if (path.resolve(sourceConfigPath) === target.currentConfigPath) {
    throw new Error(
      "Source and target Galyarder configs are the same. Pass a different source instance/config when reseeding.",
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Reseed the current worktree instance (${target.instanceId}) from ${sourceConfigPath}? This overwrites only the current worktree Galyarder instance data.`,
      initialValue: false,
    });
...
export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printGalyarderCliBanner();
  p.intro(pc.bgCyan(pc.black(" galyarder worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasGalyarderConfig: existsSync(path.resolve(worktreePath, ".galyarder", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printGalyarderCliBanner();
  p.intro(pc.bgCyan(pc.black(" galyarder worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveGalyarderEnvFile(configPath);
  const envEntries = readGalyarderEnvEntries(envPath);
  const out = {
    GALYARDER_CONFIG: configPath,
    ...(envEntries.GALYARDER_HOME ? { GALYARDER_HOME: envEntries.GALYARDER_HOME } : {}),
    ...(envEntries.GALYARDER_INSTANCE_ID ? { GALYARDER_INSTANCE_ID: envEntries.GALYARDER_INSTANCE_ID } : {}),
    ...(envEntries.GALYARDER_CONTEXT ? { GALYARDER_CONTEXT: envEntries.GALYARDER_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasGalyarderConfig)
      .map((choice) => path.resolve(choice.worktree, ".galyarder", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readGalyarderEnvEntries(resolveGalyarderEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Galyarder once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasGalyarderConfig ? "galyarder" : "no-galyarder-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".galyarder", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".galyarder", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .galyarder/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasGalyarderConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Galyarder worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasGalyarderConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Galyarder worktrees were found. Run `galyarder worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Galyarder configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Galyarder instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Galyarder instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with galyarder- if needed (created at ~/galyarder-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: GALYARDER_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: GALYARDER_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source GALYARDER_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Galyarder instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Galyarder worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with galyarder- if needed")
