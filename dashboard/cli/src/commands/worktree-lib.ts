...
export const DEFAULT_WORKTREE_HOME = "~/.galyarder-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".galyarder");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: GalyarderConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "galyarder",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    GALYARDER_HOME: paths.homeDir,
    GALYARDER_INSTANCE_ID: paths.instanceId,
    GALYARDER_CONFIG: paths.configPath,
    GALYARDER_CONTEXT: paths.contextPath,
    GALYARDER_IN_WORKTREE: "true",
    ...(branding?.name ? { GALYARDER_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { GALYARDER_WORKTREE_COLOR: branding.color } : {}),
  };
}
