/**
 * esbuild configuration for building the galyarderai CLI for npm.
 *
 * Bundles all workspace packages (@galyarderai/*) into a single file.
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@galyarderai/")) {
      externals.add(name);
    }
  }
