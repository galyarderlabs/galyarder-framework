...
Example Galyarder plugin that demonstrates:
...
pnpm galyarder plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm galyarder plugin uninstall galyarder-file-browser-example --force
```
...
- Optional: use `galyarder-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
