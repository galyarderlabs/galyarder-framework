...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".galyarder-sdk");
...
/**
 * Generate a complete Galyarder plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Galyarder plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "galyarder-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["galyarder", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@paperclipai/plugin-sdk\` and \`@paperclipai/shared\` from a local Galyarder checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.galyarder-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into Galyarder
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-galyarder-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <galyarder-sdk-path>]");
    process.exit(1);
  }
