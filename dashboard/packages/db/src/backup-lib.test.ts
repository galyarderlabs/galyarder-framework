...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("galyarder-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("galyarder-buffered-writer-");
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        ...
        "galyarder_restore_target",
      );
      const backupDir = createTempDir("galyarder-db-backup-output-");
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          backupDir,
          retentionDays: 7,
          filenamePrefix: "galyarder-test",
        });

        expect(result.backupFile).toMatch(/galyarder-test-.*\.sql$/);
  ...
  it(
    ...
    async () => {
      ...
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const backupDir = createTempDir("galyarder-db-restore-manual-");
      ...
      try {
        await fs.promises.writeFile(
          backupFile,
          [
            "-- Galyarder database backup",
            "-- Created: 2026-04-06T00:00:00.000Z",
            "",
            "BEGIN;",
            "-- galyarder statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
