...
Use when:
- You want Galyarder to run Pi (the AI coding agent) locally as the agent runtime
...
- Galyarder requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/galyarders/ and resumed with --session.
