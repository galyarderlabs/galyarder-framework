...
Use when:
- You want Galyarder to run Cursor Agent CLI locally as the agent runtime
...
- Galyarder auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$galyarder" and related skills on local runs.
- Galyarder auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
