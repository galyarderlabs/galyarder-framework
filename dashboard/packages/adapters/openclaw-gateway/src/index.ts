...
Use when:
- You want Galyarder to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Galyarder server.
...
- galyarderApiUrl (string, optional): absolute Galyarder base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/galyarder-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default galyarder)

Standard outbound payload additions:
- galyarder (object): standardized Galyarder context added to every gateway agent request
- galyarder.workspace (object, optional): resolved execution workspace for this run
- galyarder.workspaces (array, optional): additional workspace hints Galyarder exposed to the run
- galyarder.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
