...
const DEFAULT_CLIENT_VERSION = "galyarder";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "galyarder";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`galyarder:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`galyarder:issue:${input.issueId}`, input.agentId);
  }
...
function resolveGalyarderApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/galyarder-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildGalyarderEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const galyarderApiUrlOverride = resolveGalyarderApiUrlOverride(ctx.config.galyarderApiUrl);
  const galyarderEnv: Record<string, string> = {
    ...buildGalyarderEnv(ctx.agent),
    GALYARDER_RUN_ID: ctx.runId,
  };

  if (galyarderApiUrlOverride) {
    galyarderEnv.GALYARDER_API_URL = galyarderApiUrlOverride;
  }
  if (wakePayload.taskId) galyarderEnv.GALYARDER_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) galyarderEnv.GALYARDER_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) galyarderEnv.GALYARDER_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) galyarderEnv.GALYARDER_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) galyarderEnv.GALYARDER_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    galyarderEnv.GALYARDER_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  galyarderEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/galyarder-claimed-api-key.json";
  const orderedKeys = [
    "GALYARDER_RUN_ID",
    "GALYARDER_AGENT_ID",
    ...
    "GALYARDER_API_URL",
    "GALYARDER_TASK_ID",
    "GALYARDER_WAKE_REASON",
    "GALYARDER_WAKE_COMMENT_ID",
    "GALYARDER_APPROVAL_ID",
    "GALYARDER_APPROVAL_STATUS",
    "GALYARDER_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = galyarderEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = galyarderEnv.GALYARDER_API_URL ?? "<set GALYARDER_API_URL>";

  const lines = [
    "Galyarder wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `GALYARDER_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load GALYARDER_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $GALYARDER_API_KEY on every API call.",
    "- Use X-Galyarder-Run-Id: $GALYARDER_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: GALYARDER_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$GALYARDER_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$GALYARDER_COMPANY_ID/issues?assigneeAgentId=$GALYARDER_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardGalyarderPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  galyarderEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateGalyarder = parseObject(payloadTemplate.galyarder);
  const workspace = asRecord(ctx.context.galyarderWorkspace);
  const workspaces = Array.isArray(ctx.context.galyarderWorkspaces)
    ? ctx.context.galyarderWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.galyarderRuntimeServiceIntents)
    ? ctx.context.galyarderRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardGalyarder: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: galyarderEnv.GALYARDER_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.galyarderWake);
  if (Object.keys(structuredWake).length > 0) {
    standardGalyarder.wake = structuredWake;
  }

  if (workspace) {
    standardGalyarder.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardGalyarder.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardGalyarder.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "galyarder-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const galyarderEnv = buildGalyarderEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderGalyarderWakePrompt(ctx.context.galyarderWake);
  const structuredWakeJson = stringifyGalyarderWakePayload(ctx.context.galyarderWake);
  ...
  const galyarderPayload = buildStandardGalyarderPayload(ctx, wakePayload, galyarderEnv, payloadTemplate);
  ...
  agentParams.galyarder = galyarderPayload;
