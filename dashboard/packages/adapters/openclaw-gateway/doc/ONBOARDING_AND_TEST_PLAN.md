...
## Scope
This plan is now **gateway-only**. Galyarder supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Galyarder skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Galyarder task.
...
pnpm --filter galyarder typecheck
```
