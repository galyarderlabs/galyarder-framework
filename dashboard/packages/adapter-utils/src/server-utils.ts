...
const GALYARDER_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface GalyarderSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: GalyarderSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "galyarder_required",
      originLabel: "Required by Galyarder",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Galyarder",
    readOnly: false,
  };
}
...
type GalyarderWakePayload = {
  reason: string | null;
  issue: GalyarderWakeIssue | null;
  commentIds: string[];
  latestCommentId: string | null;
  comments: GalyarderWakeComment[];
...
function normalizeGalyarderWakeIssue(value: unknown): GalyarderWakeIssue | null {
...
function normalizeGalyarderWakeComment(value: unknown): GalyarderWakeComment | null {
...
export function normalizeGalyarderWakePayload(value: unknown): GalyarderWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeGalyarderWakeComment(entry))
        .filter((entry): entry is GalyarderWakeComment => Boolean(entry))
    : [];
  ...
  return {
    ...
    issue: normalizeGalyarderWakeIssue(payload.issue),
    commentIds,
...
export function stringifyGalyarderWakePayload(value: unknown): string | null {
  const normalized = normalizeGalyarderWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function renderGalyarderWakePrompt(
  value: unknown,
  ...
  const lines = resumedSession
      ? [
        "## Galyarder Resume Delta",
        "",
        "You are resuming an existing Galyarder session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "GALYARDER_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildGalyarderEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.GALYARDER_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.GALYARDER_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.GALYARDER_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.GALYARDER_API_URL = apiUrl;
  return vars;
}
...
export async function resolveGalyarderSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...GALYARDER_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listGalyarderSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<GalyarderSkillEntry[]> {
  const root = await resolveGalyarderSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `galyarder/galyarder/${entry.name}`,
        runtimeName: entry.name,
        source: path.join(root, entry.name),
        required: true,
        requiredReason: "Bundled Galyarder skills are always available for local adapters.",
      }));
  } catch {
    return [];
  }
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Galyarder skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Galyarder cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredGalyarderRuntimeSkills(value: unknown): GalyarderSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: GalyarderSkillEntry[] = [];
...
export async function readGalyarderRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<GalyarderSkillEntry[]> {
  const configuredEntries = normalizeConfiguredGalyarderRuntimeSkills(config.galyarderRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listGalyarderSkillEntries(moduleDir, additionalCandidates);
}

export async function readGalyarderSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listGalyarderSkillEntries(moduleDir);
...
export function readGalyarderSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.galyarderSkillSync;
...
function canonicalizeDesiredGalyarderSkillReference(
  reference: string,
...
export function resolveGalyarderDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readGalyarderSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredGalyarderSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeGalyarderSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.galyarderSkillSync;
  const current =
  ...
  next.galyarderSkillSync = current;
  return next;
}

export async function ensureGalyarderSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Galyarder server itself is started from
    // within a Claude Code session (e.g. `npx galyarder run` in a terminal
