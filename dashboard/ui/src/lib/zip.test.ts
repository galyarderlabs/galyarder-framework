...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("galyarder-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("galyarder-demo/COMPANY.md");
  ...
  it("reads a Galyarder Dashboard zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
        ".galyarder.yaml": "schema: galyarder/v1\n",
      },
      "galyarder-demo",
    );
  ...
  it("reads standard DEFLATE zip archives created outside Galyarder", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("galyarder-demo");
