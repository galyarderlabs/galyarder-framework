...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Galyarder data dir to use
  GALYARDER_HOST=127.0.0.1    Host passed to the onboarded server
  GALYARDER_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/galyarder-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/galyarder-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export GALYARDER_HOST="${GALYARDER_HOST:-127.0.0.1}"
export GALYARDER_PORT="${GALYARDER_PORT:-3100}"
export GALYARDER_OPEN_ON_LISTEN="${GALYARDER_OPEN_ON_LISTEN:-false}"
...
echo "GALYARDER_HOST: $GALYARDER_HOST"
echo "GALYARDER_PORT: $GALYARDER_PORT"
...
env \
  ...
  HOST="$GALYARDER_HOST" \
  PORT="$GALYARDER_PORT" \
  GALYARDER_OPEN_ON_LISTEN="$GALYARDER_OPEN_ON_LISTEN" \
  pnpm galyarderai onboard --yes --data-dir "$PC_DATA"
