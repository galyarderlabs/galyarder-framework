#!/usr/bin/env bash
#
# Kill all local Galyarder dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all galyarder dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/galyarder(-[^/]+)?/' | grep node | grep -v grep || true)

candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.galyarder/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.galyarder/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.galyarder/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/galyarder*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.galyarder/instances/*/db/postmaster.pid
    "$sibling_root"/.galyarder/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No Galyarder dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Galyarder dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Galyarder node processes..."
