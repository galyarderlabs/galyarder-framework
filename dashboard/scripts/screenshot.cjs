#!/usr/bin/env node
/**
 * Screenshot utility for Galyarder UI.
 *
 * Reads the board token from ~/.galyarder/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/galyarder-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".galyarder/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.galyarder/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Galyarder origin only
