...
COMPANY_ID="${COMPANY_ID:-${GALYARDER_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-galyarder-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-galyarder-openclaw-smoke}"
...
Provide one of:
  GALYARDER_AUTH_HEADER=\"Bearer <board-token>\"
  GALYARDER_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Galyarder OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.galyarder.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
