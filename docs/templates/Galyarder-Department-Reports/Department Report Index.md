# Department Report Index

## Purpose
This folder contains founder-readable report templates for each department in the digital enterprise.

## Templates
- [[Founder Office Report Template]]
- [[Product Report Template]]
- [[Engineering Report Template]]
- [[Security Report Template]]
- [[Growth Report Template]]
- [[Operations Report Template]]
- [[Legal-Finance Report Template]]
- [[Knowledge Report Template]]

## Usage
- Department leads use their own template for periodic reporting.
- `galyarder-specialist` can roll those reports upward into a single founder summary.
- Every report should link back to Linear execution and relevant Obsidian notes.
