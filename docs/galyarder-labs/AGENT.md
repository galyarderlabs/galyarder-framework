...
* **Final docs are P0 hard gates**: Final docs are P0 hard gates: root README.md, BRAND.md, DESIGN.md, AGENTS.md, and root agent-facing files like CLAUDE.md/GEMINI.md must align with canonical Labs rules. If instructed to read files/docs, read the source fully line-by-line with no shortcut mindset; scripts/grep can assist but never replace reading. In /home/galyarder/projects child repos, patch only obsolete root .md drift; preserve product-specific docs/identity while inheriting Labs brand/design/philosophy.
