...
Audience: Operators setting up and running agents in Galyarder

## 1. What this system does

Agents in Galyarder do not run continuously.  
...
- `hermes_local`: runs your local `hermes` CLI (`hermes-galyarder-adapter`)
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-galyarder-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Galyarder assumes the CLI is already installed and authenticated on the host machine.
...
Galyarder stores session IDs for resumable adapters.
...
Galyarder pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Galyarder surfaces this as a warning in environment tests, not a hard error.
