# ClipHub: Marketplace for Galyarder Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Galyarder configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Galyarder-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Galyarder company (agents, projects, governance) |
...
A complete Galyarder company configuration:
...
Single-agent configurations designed to plug into a Galyarder org:
...
Portable skill files that any Galyarder agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Galyarder's skill loading system
...
interface Listing {
  ...
  galyarderVersionMin: string;     // Minimum Galyarder version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Galyarder company
...
| `POST` | `/api/listings/:id/install` | Install to Galyarder company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Galyarder company (or create new)
  → ClipHub API calls Galyarder API to:
  ...
  → Redirect to Galyarder dashboard with new team running
```
...
### 5.3 Creator: Export from Galyarder → Publish

```
Running Galyarder company → "Export as Blueprint" (CLI or UI)
  → Galyarder exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Galyarder brand blue/purple)
...
| Install flow | One-click deploy to Galyarder company |
...
{
  "targetCompanyId": "uuid",         // Existing Galyarder company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `galyarder-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Galyarder
- **Backend**: Node.js API (or extend Galyarder server)
- **Database**: Postgres (can share Galyarder's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Galyarder auth (or OAuth2)

### 9.2 Integration with Galyarder

ClipHub can be:
- **Option A**: A separate app that calls Galyarder's API to install blueprints
- **Option B**: A built-in section of the Galyarder UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Galyarder UI and API.
...
- [ ] Install flow: blueprint → Galyarder company
...
- [ ] "Export from Galyarder" CLI command
