...
How to get OpenClaw running in a Docker container for local development and testing the Galyarder OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Galyarder includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-galyarder-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Galyarder host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-galyarder-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `GALYARDER_HOST_PORT` (default `3100`)
- `GALYARDER_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Galyarder deployment is `authenticated`, provide auth context:

```bash
GALYARDER_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
GALYARDER_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Galyarder server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Galyarder URL (typically `http://host.docker.internal:3100`).
- If Galyarder rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm galyarder allowed-hostname host.docker.internal
```

Then restart Galyarder and rerun the smoke script.
