...
summary: Build, package, and distribute adapters as plugins without modifying Galyarder source
---

Galyarder supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Galyarder's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `galyarder-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Galyarder | Published to npm or linked via `file:` |
| Updates | Requires Galyarder release | Independent versioning |
...
| `galyarder.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
export async function execute(
  ...
  // 2. Build environment with Galyarder vars injected
  const env = buildGalyarderEnv(agent);
...
| `buildGalyarderEnv(agent)` | Inject `GALYARDER_*` environment variables |
...
```sh
# Via the Galyarder UI
# Settings → Adapters → Install from npm → "my-galyarder-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-galyarder-adapter"}'
```
...
Local adapters are symlinked into Galyarder's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.galyarder/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-galyarder-adapter",
...
Other Galyarder users can then install your adapter by package name from the UI or API.
