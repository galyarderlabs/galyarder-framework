...
- Running a Python script that calls the Galyarder API
...
1. Galyarder spawns the configured command as a child process
2. Standard Galyarder environment variables are injected (`GALYARDER_AGENT_ID`, `GALYARDER_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Galyarder API and perform work.
