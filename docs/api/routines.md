...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Galyarder-Signature` + `X-Galyarder-Timestamp` header pair matching the trigger's signing mode.
