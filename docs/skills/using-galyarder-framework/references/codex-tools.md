...
Claude Code skills reference named agent types like `galyarder-framework:code-reviewer`.
...
|-------------------|------------------|
| `Task tool (galyarder-framework:code-reviewer)` | `spawn_agent(agent_type="worker", message=...)` with `code-reviewer.md` content |
