...
/**
 * Filter .galyarder.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterGalyarderYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://galyarder.ing) package.");
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .galyarder.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .galyarder.yaml
    const yamlPath = exportData.galyarderExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterGalyarderYaml(exportData.files[yamlPath], checkedFiles);
    }
