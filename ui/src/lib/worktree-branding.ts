...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("galyarder-worktree-enabled") !== "true") return null;

  const name = readMetaContent("galyarder-worktree-name");
  const color = normalizeHexColor(readMetaContent("galyarder-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("galyarder-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
