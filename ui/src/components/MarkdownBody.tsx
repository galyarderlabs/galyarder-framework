...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`galyarder-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="galyarder-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("galyarder-mermaid-status", error && "galyarder-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="galyarder-mermaid-source">
...
export function MarkdownBody({ children, className, style, resolveImageSrc }: MarkdownBodyProps) {
        ...
        return (
          <a
            ...
            className={cn(
              ...
              `galyarder-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "galyarder-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "galyarder-markdown prose prose-sm max-w-none break-words overflow-hidden",
