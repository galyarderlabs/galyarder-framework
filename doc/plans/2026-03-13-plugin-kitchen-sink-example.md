...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Galyarder plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Galyarder.
...
- plugin id: `galyarder.kitchen-sink-example` or `galyarder-kitchen-sink-example`

Recommendation: use `galyarder-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Galyarder event such as `issue.created`
...
Recommendation: default to a Galyarder-local endpoint or a stable public echo endpoint to avoid flaky docs.
