...
Add a single storage subsystem for Galyarder that supports:
...
- API serves attachment bytes through authenticated Galyarder endpoints.
...
- Default local path is under instance root: `~/.galyarder/instances/<instanceId>/data/storage`.
...
- `galyarder onboard` writes a valid `storage` config block by default.
- `galyarder configure --section storage` can switch between local and s3 modes.
...
- `galyarder doctor` reports actionable storage status.
