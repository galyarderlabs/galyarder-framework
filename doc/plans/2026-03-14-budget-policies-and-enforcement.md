...
Galyarder already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Galyarder should implement next.

## Product Goals

Galyarder should let operators:
...
For the next implementation pass, Galyarder should enforce these defaults:
...
Galyarder should separate subscription-included usage from billed spend:
...
Galyarder should have two threshold classes:
...
Galyarder must know that a project is budget-paused and enforce it.
...
Galyarder must also block execution before new work starts.
