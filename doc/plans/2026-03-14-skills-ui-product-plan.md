...
This document defines the product and UI plan for skill management in Galyarder.
...
- Galyarder company import/export can include skills as package content
...
3. The UI must always tell the truth about what Galyarder knows:
   - desired state in Galyarder
...
Galyarder should treat skills at two scopes:
...
- created locally inside Galyarder later
...
7. “Understand what Galyarder knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Galyarder-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Galyarder did not install.

These should be clearly marked:

- external
- not managed by Galyarder
...
- `desired_only`: Galyarder wants it, adapter does not show it yet
- `external`: adapter has it but Galyarder does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Galyarder only tracks desired state
...
- Galyarder can still track desired skills
...
- Galyarder can see adapter skills
...
- `.galyarder.yaml` may add Galyarder-specific fidelity, but should not replace the base shortname association model
...
- `.galyarder.yaml` may add Galyarder-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Galyarder should have a company-scoped skill model or managed package model representing:
...
Galyarder should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Galyarder-specific.
...
That gives Galyarder one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
