...
Galyarder's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Galyarder's API, UI, and data model | Yes |
...
modules/
  observability/
    galyarder.module.json     # manifest (required)
    src/
...
### Manifest (`galyarder.module.json`)
...
```
1. Scan modules/ directory for galyarder.module.json manifests
...
```jsonc
// galyarder.config.json (or env vars)
{
  "modules": {
  ...
  "modules": [
    {
      ...
      "repo": "github:galyarder/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.galyarder.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm galyarder store list                    # browse available modules and templates
pnpm galyarder store install <module-id>     # install a module
pnpm galyarder store import <template-id>    # import a company template
pnpm galyarder store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Galyarder instance | `server:started` |
...
4. **Module config** — `galyarder.config.json` with per-module config, env var interpolation
