...
This document defines the V1 design for a Galyarder adapter that integrates with
...
4. synthesized stdout events for Galyarder UI/CLI
...
3. **Agent callback auth to Galyarder** uses a bootstrap exchange flow (no long-lived Galyarder key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Galyarder endpoints, not full SKILL.md prompt injection.
...
- `galyarderPublicUrl` (optional override; else `GALYARDER_PUBLIC_URL` env)
...
## Galyarder Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `GALYARDER_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.galyarderPublicUrl`
2. `process.env.GALYARDER_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Galyarder credentials in prompt text.

Flow:

1. Before launch/follow-up, Galyarder mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Galyarder validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Galyarder API calls.
...
1. Prompt includes a compact instruction to fetch skills from Galyarder.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/galyarder`
   - `GET /api/skills/galyarder-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <galyarderPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Galyarder run context.
...
5. `galyarderPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Galyarder skill endpoints |
| Galyarder auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Galyarder context.
3. Add richer PR metadata surfacing in Galyarder UI.
