...
Agents need API keys to authenticate with Galyarder. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Galyarder
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Galyarder generates a token and passes it directly to the agent
...
`GALYARDER_API_URL`, `GALYARDER_AGENT_ID`, `GALYARDER_COMPANY_ID`. We need to
add a `GALYARDER_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Galyarder CLI
...
```
galyarder auth login
# Opens browser -> user confirms -> token stored at ~/.galyarder/credentials
```
...
managed by the Galyarder server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Galyarder.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Galyarder SKILL.md (or a link to it)
   - What information Galyarder needs from the agent (e.g. webhook URL, adapter
...
5. Galyarder stores the pending registration.
...
7. On approval, Galyarder provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Galyarder issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.galyarder.ing/skills/galyarder/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Galyarder should send you heartbeats",
...
1. Generate a Galyarder invite link for the company.
...
4. A Galyarder company member approves the new agent.
5. Galyarder begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `GALYARDER_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `galyarder auth login` for developer-managed remote agents.                                      |
