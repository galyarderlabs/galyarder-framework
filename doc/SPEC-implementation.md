# Galyarder V1 Implementation Spec
...
Galyarder V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.galyarder/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.galyarder/instances/default/data/storage` (`local_disk`)
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before Galyarder runtime-owned keys are injected
...
- `.galyarder.yaml` sidecar for Galyarder-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.galyarder.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and Galyarder routine fidelity in `.galyarder.yaml`
- Galyarder imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.galyarder.yaml`
