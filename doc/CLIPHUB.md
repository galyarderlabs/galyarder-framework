...
ClipHub is the public registry where people share, discover, and download Galyarder company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Galyarder what a package registry is to a programming language. Galyarder already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Galyarder — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Galyarder instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Galyarder spec. It contains:
...
**Install (fresh start):**
```
galyarder install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Galyarder instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Galyarder, export your company as a template, then publish:

```
galyarder export --template my-company
galyarder publish cliphub my-company
```
...
ClipHub is a **separate service** from Galyarder itself. Galyarder is self-hosted; ClipHub is a hosted registry that Galyarder instances talk to.
...
| **Galyarder CLI** | `galyarder install`, `galyarder publish`, `galyarder cliphub sync` — built into Galyarder |
| **Galyarder UI** | "Browse ClipHub" panel in the Galyarder web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Galyarder) |
| Backend | TypeScript + Hono (consistent with Galyarder) |
| Database | PostgreSQL |
...
4. Run `galyarder install cliphub:acme/lean-saas-shop`
5. Galyarder creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Galyarder until it works well
2. Export: `galyarder export --template my-agency`
3. Publish: `galyarder publish cliphub my-agency`
...
3. Install just that agent: `galyarder install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Galyarder

ClipHub is **not required** to use Galyarder. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Galyarder what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `galyarder install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Galyarder web UI
- [ ] `galyarder cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Galyarder's own philosophy)
