...
Galyarder V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Galyarder-shaped design.

Assumption for this document: Galyarder is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Galyarder objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Galyarder.

The main conclusion is:

- Galyarder should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Galyarder should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Galyarder should use multiple extension classes instead of one generic plugin bag:
...
If Galyarder does this well, the examples you listed become straightforward:
...
Relevant Galyarder files reviewed for current extension seams:
...
It is too dangerous for Galyarder core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Galyarder — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Galyarder should absolutely do this.
...
Galyarder should copy this discipline.
...
Galyarder should adopt this style for plugin actions, automations, and UI schemas.
...
Galyarder can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Galyarder too.

## What Galyarder Should Not Copy Directly
...
Galyarder is a control plane for an operator-managed instance with company objects.
...
Galyarder is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Galyarder should not auto-load arbitrary code from a workspace repo like `.galyarder/plugins` or project directories.
...
For Galyarder, equivalent hooks into:
...
Galyarder should not allow plugins to silently replace:
...
For Galyarder it would be risky because it combines:
...
Galyarder should require an explicit operator install step.

## Why Galyarder Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Galyarder |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Galyarder instance |
...
That means Galyarder should borrow the good ideas from `opencode` but use a stricter architecture.

## Galyarder Already Has Useful Pre-Plugin Seams

Galyarder has several extension-like seams already:
...
This is good news.
Galyarder does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Galyarder Plugin Model
...
Galyarder already has this pattern implicitly:
...
- `plugin` means integration code that talks to Galyarder through a typed plugin protocol and capability model
...
- invoke explicit Galyarder actions through the API
...
Galyarder should treat plugin installation as a global instance-level action.

Examples:

- install `@galyarder/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Galyarder objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Galyarder already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Galyarder too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@galyarder/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@paperclipai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Galyarder instance.

A `create-galyarder-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Galyarder achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Galyarder server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Galyarder's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.
...
## A Concrete SDK Shape For Galyarder
...
export default definePlugin({
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Galyarder entities
    });
    ...
    ctx.events.on("plugin.@galyarder/plugin-git.push-detected", async (event) => {
...
Any Galyarder plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Galyarder objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@galyarder/plugin-linear`)
...
- reuse core tables when the data is clearly part of Galyarder itself
...
If a concept is becoming part of Galyarder's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Galyarder itself
...
### 3. Scope plugin data to Galyarder objects before adding custom schemas

A lot of plugin data naturally hangs off existing Galyarder objects:
...
If Galyarder eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@galyarder/plugin-workspace-files`
...
Package idea: `@galyarder/plugin-terminal`
...
Package idea: `@galyarder/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Galyarder.
- Board reviews the diff after a run without leaving Galyarder.
...
- optional `events.emit` (e.g. `plugin.@galyarder/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@galyarder/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@galyarder/plugin-linear`

This plugin syncs Galyarder work with Linear. It is useful for:

- importing backlog from Linear
- linking Galyarder issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Galyarder and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Galyarder.
- Galyarder issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@galyarder/plugin-github-issues`

This plugin syncs Galyarder issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Galyarder issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Galyarder task governance
...
- Plugin settings:
  ...
  - whether new Galyarder issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Galyarder issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Galyarder.
- GitHub webhooks update status/comment state in Galyarder.
- A PR is linked back to the Galyarder issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@galyarder/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@galyarder/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Galyarder. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Galyarder issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Galyarder plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Galyarder dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Galyarder issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Galyarder core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@galyarder/plugin-runtime-processes`
...
Package idea: `@galyarder/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Galyarder. It is useful for:
...
- Failed payment spikes or churn events can generate Galyarder issues for follow-up.
...
- Stripe data should stay additive to Galyarder core
...
- `@paperclipai/plugin-test-harness` and `create-galyarder-plugin` starter template
...
Galyarder should not implement "an OpenCode-style generic in-process hook system."
Galyarder should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Galyarder
...
8. Ship `@paperclipai/plugin-test-harness` and `create-galyarder-plugin` starter template.
