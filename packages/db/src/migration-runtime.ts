...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://galyarder:galyarder@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "galyarder");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://galyarder:galyarder@127.0.0.1:${preferredPort}/galyarder`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://galyarder:galyarder@127.0.0.1:${port}/galyarder`,
  ...
  const adminConnectionString = `postgres://galyarder:galyarder@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "galyarder");

  return {
    connectionString: `postgres://galyarder:galyarder@127.0.0.1:${selectedPort}/galyarder`,
