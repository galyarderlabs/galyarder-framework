...
Official TypeScript SDK for Galyarder plugin authors.
...
- Plugin UI bundles run as same-origin JavaScript inside the main Galyarder app. They can call ordinary Galyarder HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Galyarder instance at runtime.
...
```bash
galyarder-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```
...
- `GET /__galyarder__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__galyarder__/events` streams `reload` SSE events on UI build changes
