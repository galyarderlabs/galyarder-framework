/**
 * Core types for the Galyarder plugin worker-side SDK.
 *
...
export interface ScopeKey {
  /** What kind of Galyarder object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Galyarder UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Galyarder domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Galyarder domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Galyarder secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Galyarder secret provider (e.g. `"MY_API_KEY"`).
   *
