...
type GalyarderSprite = (typeof GALYARDER_SPRITES)[number];
...
function spriteSize(sprite: GalyarderSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = GALYARDER_SPRITES[(baseCol + baseRow) % GALYARDER_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = GALYARDER_SPRITES[Math.floor(Math.random() * GALYARDER_SPRITES.length)]!;
