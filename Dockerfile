...
RUN usermod -u $USER_UID --non-unique node \
  ...
  && usermod -g $USER_GID -d /galyarder node
...
ENV NODE_ENV=production \
  ...
  PORT=3100 \
  SERVE_UI=true \
  GALYARDER_HOME=/galyarder \
  GALYARDER_INSTANCE_ID=default \
  USER_UID=${USER_UID} \
  USER_GID=${USER_GID} \
  GALYARDER_CONFIG=/galyarder/instances/default/config.json \
