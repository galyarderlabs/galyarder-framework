...
GALYARDER_HOST_PORT="${GALYARDER_HOST_PORT:-3100}"
GALYARDER_HOST_FROM_CONTAINER="${GALYARDER_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.galyarder-openclaw.override.yml"
...
detect_galyarder_base_url() {
  ...
  for candidate in "$GALYARDER_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${GALYARDER_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${GALYARDER_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
galyarder_base_url="$(detect_galyarder_base_url || true)"
...
Galyarder URL for OpenClaw container:
EOF
  if [[ -n "$galyarder_base_url" ]]; then
    cat <<EOF
  $galyarder_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${GALYARDER_HOST_PORT}
  (Do not use http://127.0.0.1:${GALYARDER_HOST_PORT} inside the container.)
  If Galyarder rejects the host, run on host machine:
    pnpm galyarderai allowed-hostname host.docker.internal
  Then restart Galyarder and re-run this script.
EOF
  fi
  cat <<EOF
