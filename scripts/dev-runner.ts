...
const devServerStatusFilePath = path.join(repoRoot, ".galyarder", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".galyarder/dev-server-status.json",
]);
...
const env: NodeJS.ProcessEnv = {
  ...process.env,
  GALYARDER_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.GALYARDER_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.GALYARDER_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  env.GALYARDER_MIGRATION_PROMPT ??= "never";
  env.GALYARDER_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.GALYARDER_DEPLOYMENT_MODE = "authenticated";
  env.GALYARDER_DEPLOYMENT_EXPOSURE = "private";
  env.GALYARDER_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[galyarder] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[galyarder] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[galyarder] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[galyarder] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.GALYARDER_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[galyarder] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  ...
  if (result.code !== 0) {
    console.error("[galyarder] plugin sdk build failed");
