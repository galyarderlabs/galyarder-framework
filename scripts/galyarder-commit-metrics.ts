...
const DEFAULT_QUERY = "\"Co-Authored-By: Galyarder <noreply@galyarder.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/galyarder-commit-metrics-cache.json");
...
const GALYARDER_EMAIL = "noreply@galyarder.ing";
const GALYARDER_NAME = "galyarder";
...
function printHelp() {
  console.log(`Usage: tsx scripts/galyarder-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === GALYARDER_EMAIL) || (displayName && displayName.toLowerCase() === GALYARDER_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Galyarder commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "galyarder-commit-metrics",
