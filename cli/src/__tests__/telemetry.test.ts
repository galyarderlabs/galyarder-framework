...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.GALYARDER_HOME = path.join(root, "home");
    process.env.GALYARDER_INSTANCE_ID = "telemetry-test";
