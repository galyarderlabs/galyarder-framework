...
import { ApiRequestError, GalyarderApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Galyarder config file")
    .option("-d, --data-dir <path>", "Galyarder data directory root (isolates state from ~/.galyarder)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Galyarder API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.GALYARDER_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.GALYARDER_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.GALYARDER_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set GALYARDER_COMPANY_ID, or set context profile companyId via `galyarder context set`.",
    );
  }

  const api = new GalyarderApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.GALYARDER_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.GALYARDER_SERVER_PORT || "");
