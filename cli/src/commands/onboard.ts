...
type OnboardDefaults = Pick<GalyarderConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "GALYARDER_PUBLIC_URL",
  "DATABASE_URL",
  "GALYARDER_DB_BACKUP_ENABLED",
  "GALYARDER_DB_BACKUP_INTERVAL_MINUTES",
  "GALYARDER_DB_BACKUP_RETENTION_DAYS",
  "GALYARDER_DB_BACKUP_DIR",
  "GALYARDER_DEPLOYMENT_MODE",
  "GALYARDER_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "GALYARDER_ALLOWED_HOSTNAMES",
  "GALYARDER_AUTH_BASE_URL_MODE",
  "GALYARDER_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "GALYARDER_STORAGE_PROVIDER",
  "GALYARDER_STORAGE_LOCAL_DIR",
  "GALYARDER_STORAGE_S3_BUCKET",
  "GALYARDER_STORAGE_S3_REGION",
  "GALYARDER_STORAGE_S3_ENDPOINT",
  "GALYARDER_STORAGE_S3_PREFIX",
  "GALYARDER_STORAGE_S3_FORCE_PATH_STYLE",
  "GALYARDER_SECRETS_PROVIDER",
  "GALYARDER_SECRETS_STRICT_MODE",
  "GALYARDER_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveGalyarderInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.GALYARDER_PUBLIC_URL?.trim() ||
    process.env.GALYARDER_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.GALYARDER_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.GALYARDER_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.GALYARDER_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.GALYARDER_ALLOWED_HOSTNAMES
    ? process.env.GALYARDER_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.GALYARDER_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.GALYARDER_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.GALYARDER_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.GALYARDER_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.GALYARDER_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.GALYARDER_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.GALYARDER_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.GALYARDER_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.GALYARDER_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.GALYARDER_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.GALYARDER_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.GALYARDER_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.GALYARDER_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.GALYARDER_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.GALYARDER_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "GALYARDER_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<GalyarderConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printGalyarderCliBanner();
  p.intro(pc.bgCyan(pc.black(" galyarder onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveGalyarderInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: GalyarderConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Galyarder install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("galyarder configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("GALYARDER_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.GALYARDER_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("GALYARDER_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("GALYARDER_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: GALYARDER_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("galyarder run")}`,
        `Reconfigure later: ${pc.cyan("galyarder configure")}`,
        `Diagnose setup: ${pc.cyan("galyarder doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.GALYARDER_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Galyarder setup is ready.");
    return;
  }
  ...
  let llm: GalyarderConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        ...
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `galyarder doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("galyarder run")}`,
        `Then: ${pc.cyan("galyarder auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
