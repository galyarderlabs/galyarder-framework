const FAVICON_BLOCK_START = "<!-- GALYARDER_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- GALYARDER_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- GALYARDER_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- GALYARDER_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.GALYARDER_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.GALYARDER_WORKTREE_NAME) ?? nonEmpty(env.GALYARDER_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.GALYARDER_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="galyarder-worktree-enabled" content="true" />',
    `<meta name="galyarder-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="galyarder-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="galyarder-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
