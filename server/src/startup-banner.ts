...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.GALYARDER_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.GALYARDER_AGENT_JWT_SECRET === "string" ? parsed.GALYARDER_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm galyarder onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveGalyarderEnvPath();
