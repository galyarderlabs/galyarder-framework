...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Galyarder Dashboard worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.GALYARDER_IN_WORKTREE !== "true") return null;

  const configPath = resolveGalyarderConfigPath(overrideConfigPath);
  const envPath = resolveGalyarderEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.GALYARDER_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.GALYARDER_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.GALYARDER_HOME) ??
      nonEmpty(env.GALYARDER_WORKTREES_DIR) ??
      "~/.galyarder-worktrees",
  );
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".galyarder", "config.json");
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.GALYARDER_HOME = context.homeDir;
  process.env.GALYARDER_INSTANCE_ID = context.instanceId;
  process.env.GALYARDER_CONFIG = context.configPath;
  process.env.GALYARDER_CONTEXT = context.contextPath;
  process.env.GALYARDER_WORKTREE_NAME = context.worktreeName;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    GALYARDER_HOME: context.homeDir,
    GALYARDER_INSTANCE_ID: context.instanceId,
    GALYARDER_CONFIG: context.configPath,
    GALYARDER_CONTEXT: context.contextPath,
    GALYARDER_IN_WORKTREE: "true",
    GALYARDER_WORKTREE_NAME: context.worktreeName,
  };
