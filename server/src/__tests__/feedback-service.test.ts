...
async function startTempDatabase() {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-feedback-service-"));
  ...
  const adminConnectionString = `postgres://galyarder:galyarder@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "galyarder");
  const connectionString = `postgres://galyarder:galyarder@127.0.0.1:${port}/galyarder`;
...
describe("feedbackService.saveIssueVote", () => {
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-feedback-instructions-"));
    ...
    await db.insert(companySkills).values([
      {
        id: randomUUID(),
        companyId,
        key: "galyarder/galyarder/galyarder",
        slug: "galyarder",
        name: "Galyarder",
        markdown: "# Galyarder Dashboard",
        sourceType: "catalog",
        sourceLocator: null,
        sourceRef: null,
    ...
    await db.insert(agents).values({
      ...
      status: "active",
      adapterType: "codex_local",
      adapterConfig: writeGalyarderSkillSyncPreference(
        {
          model: "gpt-5.4",
        ...
        ["galyarder/galyarder/galyarder", "octo/research/public-skill"],
      ),
      runtimeConfig: {
        heartbeat: {
          enabled: true,
  ...
  it("builds a detailed sanitized shared bundle with issue and agent context", async () => {
    ...
    expect(trace?.schemaVersion).toBe("galyarder-feedback-envelope-v2");
    expect(trace?.bundleVersion).toBe("galyarder-feedback-bundle-v2");
  ...
  it("keeps earlier local votes local when a later vote enables sharing", async () => {
    ...
    expect(localTrace?.payloadVersion).toBe("galyarder-feedback-v1");
    ...
    expect(sharedTrace?.payloadVersion).toBe("galyarder-feedback-v1");
  });

  it("captures Claude project session artifacts as full traces", async () => {
    const claudeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-feedback-claude-"));
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    const opencodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-feedback-opencode-"));
    ...
    vi.stubEnv("GALYARDER_OPENCODE_STORAGE_DIR", opencodeRoot);
    const uploadTraceBundle = vi.fn().mockResolvedValue({ objectKey: "feedback-traces/test.json" });
    const flushingSvc = feedbackService(db, {
      shareClient: {
        uploadTraceBundle,
  ...
  it("flushes pending shared traces into configured object storage and marks them sent", async () => {
    ...
    expect(uploadTraceBundle.mock.calls[0]?.[0]).toMatchObject({
      ...
      envelope: {
        destination: "galyarder_labs_feedback_v1",
        exportId: traces[0]?.exportId,
      },
    });
  });
