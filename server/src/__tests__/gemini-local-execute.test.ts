...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects galyarder env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the galyarder heartbeat.");
      expect(promptArg).toContain("Galyarder runtime note:");
      ...
      expect(invocationPrompt).toContain("Galyarder API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-gemini-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(promptArg).toContain("## Galyarder Dashboard Resume Delta");
      ...
      expect(promptArg).not.toContain("Follow the galyarder heartbeat.");
    } finally {
      if (previousHome === undefined) {
        delete process.env.HOME;
      } else {
