...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
        ...
        buildContext(
          ...
          },
          {
            ...
            context: {
              ...
              galyarderRuntimeServiceIntents: [
                {
                  name: "preview",
                  lifecycle: "ephemeral",
                },
      ...
      expect(payload?.sessionKey).toBe("galyarder:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("GALYARDER_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("GALYARDER_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Galyarder Dashboard Wake Payload");
      ...
      expect(payload?.galyarder).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },
