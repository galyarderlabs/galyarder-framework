...
describe("agent local JWT", () => {
  const secretEnv = "GALYARDER_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "GALYARDER_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "GALYARDER_AGENT_JWT_ISSUER";
  const audienceEnv = "GALYARDER_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when GALYARDER_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "galyarder";
    process.env[audienceEnv] = "galyarder-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
