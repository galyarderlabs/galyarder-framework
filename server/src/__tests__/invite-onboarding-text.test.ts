...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Galyarder OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Galyarder Dashboard base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("galyarderApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.galyarderApiUrl");
    expect(text).toContain("~/.openclaw/workspace/galyarder-claimed-api-key.json");
    expect(text).toContain("GALYARDER_API_KEY");
