...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Galyarder Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@galyarder.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("galyarder-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `galyarder-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "galyarder-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "galyarder-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "galyarder-close-check",
      baseRef: "main",
      metadata: {
  ...
  it("shows inherited shared project runtime services on shared execution workspaces without duplicating old history", async () => {
    ...
    const reuseKey = `project_workspace:${projectWorkspaceId}:galyarder-dev`;
    ...
    await db.insert(projectWorkspaces).values({
      ...
      metadata: {
        runtimeConfig: {
          ...
          workspaceRuntime: {
            services: [{ name: "galyarder-dev", command: "pnpm dev" }],
          },
        },
      },
    });
