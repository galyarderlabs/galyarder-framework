...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const galyarderDir = path.join(worktreeRoot, ".galyarder");
    const configPath = path.join(galyarderDir, "config.json");
    const envPath = path.join(galyarderDir, ".env");
    const sharedRoot = path.join(tempRoot, ".galyarder", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".galyarder-worktrees");

    await fs.mkdir(galyarderDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Galyarder Dashboard environment variables",
        "GALYARDER_IN_WORKTREE=true",
        "GALYARDER_WORKTREE_NAME=PAP-884-ai-commits-component",
        "GALYARDER_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.GALYARDER_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.GALYARDER_WORKTREES_DIR = isolatedHome;
    delete process.env.GALYARDER_HOME;
    delete process.env.GALYARDER_INSTANCE_ID;
    delete process.env.GALYARDER_CONFIG;
    delete process.env.GALYARDER_CONTEXT;
    ...
    expect(repairedEnv).toContain(`GALYARDER_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('GALYARDER_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`GALYARDER_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`GALYARDER_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('GALYARDER_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.GALYARDER_HOME).toBe(isolatedHome);
    expect(process.env.GALYARDER_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Galyarder Dashboard environment variables",
        "GALYARDER_IN_WORKTREE=true",
        "GALYARDER_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.GALYARDER_IN_WORKTREE = "true";
    process.env.GALYARDER_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.GALYARDER_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".galyarder-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".galyarder", "worktrees");
    ...
    const galyarderDir = path.join(currentWorktreeRoot, ".galyarder");
    const configPath = path.join(galyarderDir, "config.json");
    const envPath = path.join(galyarderDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".galyarder", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.GALYARDER_IN_WORKTREE = "true";
    process.env.GALYARDER_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.GALYARDER_HOME = isolatedHome;
    process.env.GALYARDER_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.GALYARDER_CONFIG = configPath;
