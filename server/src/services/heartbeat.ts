...
const DEFERRED_WAKE_CONTEXT_KEY = "_galyarderWakeContext";
const WAKE_COMMENT_IDS_KEY = "wakeCommentIds";
const GALYARDER_WAKE_PAYLOAD_KEY = "galyarderWake";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__galyarder_repo_only__";
...
function enrichWakeContextSnapshot(input: {
  ...
  if (wakeCommentIds.length > 0) {
    ...
    delete contextSnapshot[GALYARDER_WAKE_PAYLOAD_KEY];
...
export function mergeCoalescedContextSnapshot(
  ...
  if (mergedCommentIds.length > 0) {
    ...
    delete merged[GALYARDER_WAKE_PAYLOAD_KEY];
  }
  return merged;
}

async function buildGalyarderWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  issueSummary?:
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    const issueRef = issueContext
      ...
      : null;
    const galyarderWakePayload = await buildGalyarderWakePayload({
      db,
      companyId: agent.companyId,
    ...
    if (galyarderWakePayload) {
      context[GALYARDER_WAKE_PAYLOAD_KEY] = galyarderWakePayload;
    } else {
      delete context[GALYARDER_WAKE_PAYLOAD_KEY];
    }
    ...
    const runtimeConfig = {
      ...resolvedConfig,
      galyarderRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.galyarderWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.galyarderRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.galyarderRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.galyarderSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.galyarderSessionRotationReason = sessionCompaction.reason;
      context.galyarderPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.galyarderSessionHandoffMarkdown;
      delete context.galyarderSessionRotationReason;
      delete context.galyarderPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.galyarderRuntimeServices = runtimeServices;
        context.galyarderRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[galyarder] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected GALYARDER_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.galyarderRuntimeServices = combinedRuntimeServices;
        context.galyarderRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[galyarder] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
      ...
      if (finalizedRun) {
        ...
        if (issueId && outcome === "succeeded") {
          try {
            ...
            await onLog(
              "stderr",
              `[galyarder] Failed to post run summary comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
