...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".galyarder.yaml" || normalized === ".galyarder.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const galyarder = isPlainRecord(metadata?.galyarder) ? metadata?.galyarder as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.galyarderSkillKey)
    ?? asString(galyarder?.skillKey)
    ?? asString(galyarder?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "galyarder_bundled") {
    return `galyarder/galyarder/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "galyarder" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .galyarder.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .galyarder.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .galyarder.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .galyarder.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Galyarder Dashboard will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .galyarder.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .galyarder.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .galyarder.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .galyarder.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .galyarder.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .galyarder.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .galyarder.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .galyarder.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .galyarder.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[galyarderExtensionPath];
  if (selectedFiles.has(galyarderExtensionPath) && typeof extensionEntry === "string") {
    filtered[galyarderExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findGalyarderExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".galyarder.yaml"] === "string") return ".galyarder.yaml";
  if (typeof files[".galyarder.yml"] === "string") return ".galyarder.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.galyarder.yaml") || entry.endsWith("/.galyarder.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "galyarder_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "galyarder/galyarder",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/galyarder/galyarder/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "galyarder_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.galyarderSkillKey = skill.key;
  metadata.galyarder = {
    ...(isPlainRecord(metadata.galyarder) ? metadata.galyarder : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const galyarderExtensionPath = findGalyarderExtensionPath(normalizedFiles);
  const galyarderExtension = galyarderExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, galyarderExtensionPath) ?? "")
    : {};
  const galyarderCompany = isPlainRecord(galyarderExtension.company) ? galyarderExtension.company : {};
  const galyarderSidebar = normalizePortableSidebarOrder(galyarderExtension.sidebar);
  const galyarderAgents = isPlainRecord(galyarderExtension.agents) ? galyarderExtension.agents : {};
  const galyarderProjects = isPlainRecord(galyarderExtension.projects) ? galyarderExtension.projects : {};
  const galyarderTasks = isPlainRecord(galyarderExtension.tasks) ? galyarderExtension.tasks : {};
  const galyarderRoutines = isPlainRecord(galyarderExtension.routines) ? galyarderExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(galyarderCompany.brandColor),
      logoPath: asString(galyarderCompany.logoPath) ?? asString(galyarderCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof galyarderCompany.requireBoardApprovalForNewAgents === "boolean"
          ? galyarderCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof galyarderCompany.feedbackDataSharingEnabled === "boolean"
          ? galyarderCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof galyarderCompany.feedbackDataSharingConsentAt === "string"
          ? galyarderCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(galyarderCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(galyarderCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: galyarderSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(galyarderAgents[slug]) ? galyarderAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(galyarderProjects[slug]) ? galyarderProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(galyarderTasks[slug]) ? galyarderTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(galyarderRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(galyarderRoutines[slug]) ? galyarderRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".galyarder.yaml" ||
          relative === ".galyarder.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const galyarderAgentsOut: Record<string, Record<string, unknown>> = {};
    const galyarderProjectsOut: Record<string, Record<string, unknown>> = {};
    const galyarderTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const galyarderRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readGalyarderSkillSyncPreference(
        ...
        galyarderAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      galyarderProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      galyarderTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      galyarderRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const galyarderExtensionPath = ".galyarder.yaml";
    const galyarderAgents = Object.fromEntries(
      Object.entries(galyarderAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const galyarderProjects = Object.fromEntries(
      Object.entries(galyarderProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const galyarderTasks = Object.fromEntries(
      Object.entries(galyarderTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const galyarderRoutines = Object.fromEntries(
      Object.entries(galyarderRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[galyarderExtensionPath] = buildYamlFile(
      {
        schema: "galyarder/v1",
        ...
        agents: Object.keys(galyarderAgents).length > 0 ? galyarderAgents : undefined,
        projects: Object.keys(galyarderProjects).length > 0 ? galyarderProjects : undefined,
        tasks: Object.keys(galyarderTasks).length > 0 ? galyarderTasks : undefined,
        routines: Object.keys(galyarderRoutines).length > 0 ? galyarderRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, galyarderExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeGalyarderSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
