...
import { resolveHomeAwarePath, resolveGalyarderInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "galyarder-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "galyarder-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "galyarder-feedback-v1";
const FEEDBACK_DESTINATION = "galyarder_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "galyarder_run" &&
    file.source !== "galyarder_run_events" &&
    file.source !== "galyarder_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.GALYARDER_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readGalyarderSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let galyarderRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      galyarderRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "galyarder/run.json",
        contentType: "application/json",
        source: "galyarder_run",
        contents: `${JSON.stringify(galyarderRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.galyarderRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "galyarder/run-events.json",
        contentType: "application/json",
        source: "galyarder_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "galyarder/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "galyarder_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.galyarderRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
