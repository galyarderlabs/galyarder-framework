...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".galyarder", "dist", "node_modules"]);
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.GALYARDER_WORKSPACE_CWD = input.worktreePath;
  env.GALYARDER_WORKSPACE_PATH = input.worktreePath;
  env.GALYARDER_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.GALYARDER_WORKSPACE_BRANCH = input.branchName;
  env.GALYARDER_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.GALYARDER_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.GALYARDER_WORKSPACE_SOURCE = input.base.source;
  env.GALYARDER_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.GALYARDER_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.GALYARDER_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.GALYARDER_PROJECT_ID = input.base.projectId ?? "";
  env.GALYARDER_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.GALYARDER_AGENT_ID = input.agent.id ?? "";
  env.GALYARDER_AGENT_NAME = input.agent.name;
  env.GALYARDER_COMPANY_ID = input.agent.companyId;
  env.GALYARDER_ISSUE_ID = input.issue?.id ?? "";
  env.GALYARDER_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.GALYARDER_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.GALYARDER_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.GALYARDER_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.GALYARDER_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.GALYARDER_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.GALYARDER_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.GALYARDER_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.GALYARDER_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.GALYARDER_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.GALYARDER_PROJECT_ID = input.workspace.projectId ?? "";
  env.GALYARDER_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.GALYARDER_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".galyarder", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Galyarder", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
