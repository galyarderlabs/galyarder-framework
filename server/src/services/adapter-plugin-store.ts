 ...
 * ~/.galyarder/adapter-plugins.json. This is the source of truth for which
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-galyarder-adapter") */
  packageName: string;
...
const GALYARDER_DIR = path.join(os.homedir(), ".galyarder");
const ADAPTER_PLUGINS_DIR = path.join(GALYARDER_DIR, "adapter-plugins");
const ADAPTER_PLUGINS_STORE_PATH = path.join(GALYARDER_DIR, "adapter-plugins.json");
const ADAPTER_SETTINGS_PATH = path.join(GALYARDER_DIR, "adapter-settings.json");
...
function ensureDirs(): void {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "galyarder-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Galyarder Dashboard external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
}
